...
public:
  ...
  void finalize(llvm::raw_ostream &OS) {
    ...
    if (llvm::compression::ZlibCompressionAlgorithm().supported()) {
      ...
      llvm::compression::ZlibCompressionAlgorithm().compress(
...
llvm::Expected<StringTableIn> readStringTable(llvm::StringRef Data) {
  ...
  if (UncompressedSize == 0) // No compression
    Uncompressed = R.rest();
  else if (llvm::compression::ZlibCompressionAlgorithm().supported()) {
    ...
    if (llvm::Error E =
            llvm::compression::ZlibCompressionAlgorithm().decompress(
